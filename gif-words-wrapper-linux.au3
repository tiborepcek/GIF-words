...
For $i = 1 To _FileCountLines($fileRead)
	...
	FileWriteLine($fileWrite, "convert -font " & $txtFont & " -pointsize " & $txtPointsize & " -kerning " & $txtKerning & " -interline-spacing " & $txtInterlineSpacing & " -size " & $imgSize & " -gravity " & $imgGravity & " caption:""" & StringStripWS($value[1], 3) & "\n" & StringStripWS($value[2], 3) & """ " & $imgName & $imgNumber & ".gif")
Next
